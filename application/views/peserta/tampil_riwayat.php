<div class="container">
	...
	<table style="width: 100%;" class="table table-hover table-responsive table-bordered" >
		<tr>
			...
			<th class="text-center table-secondary">Event</th>
			<th class="text-center table-secondary">Sub Event</th>
			<th class="text-center table-secondary">Kategori</th>
			<th class="text-center table-secondary">Nama Tim</th>
		...
		<?php 
	      ...
	      foreach($riwayat as $riw) : ?>
		    <tr>
			    ...
			    <?php if ($riw->status == '1'): ?>
				    <td align="center">
			    		<button onclick="window.location.href='<?php echo base_url('peserta/riwayat/edit_riwayat1/'.$riw->id_peserta) ?>'" class="btn btn-sm btn-warning">Detail & Edit</button>
				    </td>
				    <td align="center">
				...
				<?php elseif ($riw->status == '2' or '3'): ?>
					 <td align="center">
			    		<button onclick="window.location.href='<?php echo base_url('peserta/riwayat/edit_riwayat1/'.$riw->id_peserta) ?>'" class="btn btn-sm btn-warning">View</button>
				    </td>
				    <td align="center">
				    	<button disabled class="btn btn-sm btn-danger">Kirim Data</button>
				    </td>
			    <?php endif ?>
		    </tr>
		<?php endforeach; ?>
